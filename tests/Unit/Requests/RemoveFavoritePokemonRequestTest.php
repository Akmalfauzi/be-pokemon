...
class RemoveFavoritePokemonRequestTest extends TestCase
{
    use RefreshDatabase;

    private function createRequest(array $data = [], $routeParameter = null): RemoveFavoritePokemonRequest
    {
        $request = new RemoveFavoritePokemonRequest();
        $request->merge($data);

        // Set up route parameter if provided
        if ($routeParameter !== null) {
            $request->setRouteResolver(function () use ($routeParameter) {
                $route = new \Illuminate\Routing\Route(['DELETE'], '/test/{pokemon}', []);
    ...
    {
        $request = $this->createRequest(['pokemon_id' => 'pikachu']);
        $rules = $request->rules();
        $validator = validator()->make($request->all(), $rules);
    ...
    public function test_validation_passes_with_positive_pokemon_id(): void
    {
        $request = $this->createRequest(['pokemon_id' => 1]);
        $rules = $request->rules();
        $validator = validator()->make($request->all(), $rules);
    ...
    public function test_validation_passes_with_large_pokemon_id(): void
    {
        $request = $this->createRequest(['pokemon_id' => 999999]);
        $rules = $request->rules();
        $validator = validator()->make($request->all(), $rules);
    ...
    public function test_route_parameter_validation_works(): void
    {
        $request = $this->createRequest(['pokemon_id' => 25]);
        $rules = $request->rules();
        $validator = validator()->make($request->all(), $rules);
